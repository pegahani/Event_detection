...
class TM_NMF:
    def __init__(self, all_documents, num_topics, num_top_words, min_df, max_df):
        ...
        self.file = open('./result/Topic_modeling_nmf.txt', 'w')
    ...
    def show_corpus_vs_topics(self):
        ...
        self.file.write('\n')
        self.file.write("top topics of each corpus\n")
        corpus = corpus_names

        for i in range(len(doctopic)):
            top_topics = np.argsort(doctopic[i, :])[::-1][0:1]
            top_topics_str = ' '.join(str(t) for t in top_topics)
            self.file.write("{}: {}".format(corpus[i], top_topics_str) + '\n')
            self.file.flush()
        return
...
#
# nmf_instance = TM_NMF(filenames, num_topics= 3, num_top_words= 10)
